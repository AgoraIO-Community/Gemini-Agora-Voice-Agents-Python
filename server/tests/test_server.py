...
def test_lazy_agent_initialization_keeps_config_available(client, server_module, monkeypatch):
    server_module.agent = None
    assert client.get("/get_config").status_code == 200

    def fail_to_initialize_agent():
        raise RuntimeError("GOOGLE_API_KEY is required for GeminiSTT")

    monkeypatch.setattr(server_module, "_get_agent", fail_to_initialize_agent)
    assert (
        client.post(
            "/startAgent", json={"channelName": "c", "rtcUid": 1, "userUid": 2}
        ).status_code
