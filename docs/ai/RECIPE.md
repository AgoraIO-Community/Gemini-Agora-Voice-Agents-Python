...
stable_contracts:
  - id: env.required
    summary: AGORA_APP_ID, AGORA_APP_CERTIFICATE, and GOOGLE_API_KEY are required by FastAPI; AGENT_BACKEND_URL is required by deployed web rewrites.
  - id: api.core-routes
...
- FastAPI owns token generation, `AGORA_APP_CERTIFICATE`, `GOOGLE_API_KEY`, optional alternate-provider keys, and agent lifecycle.
...
- Exact provider model names, VAD timing, voice IDs, and prompt text, as long as they remain documented extension points.
