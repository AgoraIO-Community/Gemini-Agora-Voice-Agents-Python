...
`server.py` derives the `server/` directory from `__file__` and loads `server/.env.local` then `server/.env`. Running from the repo root still finds those files; missing `AGORA_APP_ID`, `AGORA_APP_CERTIFICATE`, or `GOOGLE_API_KEY` leaves `agent = None` and routes return `500`.
